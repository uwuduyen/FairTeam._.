...
app.delete("/api/tasks/:id",auth,(req,res)=>{
 const t=get("SELECT * FROM tasks WHERE id=?",[req.params.id]);if(!t||t.assignee_id!==req.session.userId||t.status!=="verified")return res.status(403).json({error:"Only your verified tasks can be deleted"});
 run("DELETE FROM calendar_events WHERE task_id=?",[t.id]);run("DELETE FROM task_history WHERE task_id=?",[t.id]);run("DELETE FROM tasks WHERE id=?",[t.id]);res.json({ok:true});
});
app.post("/api/tasks/:id/effort-suggest",auth,(req,res)=>{
 const t=get("SELECT * FROM tasks WHERE id=?",[req.params.id]);if(!t||t.assignee_id!==req.session.userId)return res.status(403).json({error:"Only the assignee can suggest effort"});
 const effort=Math.min(5,Math.max(1,Number(req.body.effort||t.effort)));run("UPDATE tasks SET effort=? WHERE id=?",[effort,t.id]);run("INSERT INTO task_history(task_id,user_id,action,detail) VALUES(?,?,?,?)",[t.id,req.session.userId,"effort_suggested",`Suggested effort: ${effort}/5. ${req.body.reason||""}`]);res.json({ok:true});
});
...
app.get("/api/teams/:id/contribution",auth,(req,res)=>{
 ...
 const result=members.map(m=>{
   ...
   const task_factor=effort; const quality_factor=quality; const reliability_factor=reliability; const peer_factor=peer; return {...m,total,verified,score,task_factor,quality_factor,reliability_factor,peer_factor};
 });
